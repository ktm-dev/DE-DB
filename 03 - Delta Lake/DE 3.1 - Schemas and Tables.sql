...
-- MAGIC print(type(schema_default_location))
-- MAGIC print(*schema_default_location,sep='\n')
...
DESCRIBE  managed_table_in_db_with_custom_location;
...
CREATE   table temp_delays1 USING CSV OPTIONS (
  path = '${da.paths.datasets}/flights/departuredelays.csv',
  header = "true",
  mode = "FAILFAST" -- abort file parsing with a RuntimeException if any malformed lines are encountered
);
